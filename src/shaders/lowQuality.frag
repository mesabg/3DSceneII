...
void main(void){
    ...
    for (int i=0; i<NLIGHTS; i++){
        if (u_normal_mapped == 1){
            if (u_light[i].type.x == 1) L = normalize(position_tangent); 
            ...
            texture_coord = ParallaxMapping(o_texture_coord, L);
        }else{
    ...
    if (colored > 0.0) resultingColor = mix(color, texture(u_texture, texture_coord), 0.7);
    else resultingColor = vec4(0, 0, 0, 1);*/

    resultingColor = texture(u_texture, o_texture_coord);
}